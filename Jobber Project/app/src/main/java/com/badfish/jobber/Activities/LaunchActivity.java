        ...
        FirebaseMessaging.getInstance().getToken().addOnCompleteListener(new OnCompleteListener<String>() {
            ...
            public void onComplete(@NonNull Task<String> task) {
                if (!task.isSuccessful()) {
                    Toast.makeText(LaunchActivity.this, "Fetching FCM registration token failed", Toast.LENGTH_SHORT).show();
                    Toast.makeText(LaunchActivity.this, task.getException().getMessage(), Toast.LENGTH_LONG).show();


                    return;
                }
                Toast.makeText(LaunchActivity.this, "Token fetching succesful", Toast.LENGTH_SHORT).show();
                String tokenId = task.getResult();
                SharedPreferences pref = getSharedPreferences("Token", Context.MODE_PRIVATE);
                SharedPreferences pref2 = getSharedPreferences("MyPrefsFile", Context.MODE_PRIVATE);
                pref2.edit().putString("token",tokenId).commit();
